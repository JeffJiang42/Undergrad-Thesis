...
$e_i^-$ is the right action of $-e_i^+$.  In addition, the Dirac operator has the
special property of being an \ib{elliptic operator}. We will not define this property,
but we will use some consequences of elliptic theory and functional analysis to deduce
certain properties of $D$. In particular we will use three properties of $D$.
%TODO talk about the metric on sections via the fiber metric. Need compactness and
% skew self adjointness?
\begin{enumerate}
  \item $-D^2$ is positive, i.e. all the eigenvalues are positive. This does not need
  elliptic theory to be verified.
  \item $D^2$ has a discrete spectrum -- its eigenvalues form a discrete subset
  of $\R$.
  \item $D$ has finite dimensional eigenspace. In particular, $D$ has a finite
  dimensional kernel.
\end{enumerate}
%

\iffalse
With these properties in had, we make a few observations. We first note that $\ker D$
is itself a Clifford module with respect to the right action of $\Cliff_{n,0}(\R)$,
since $D$ commutes with the right Clifford action. Then since it is finite dimensional,
it determines an element of the $K$-Theory group $KO^n(\mathrm{pt})$. In addition, $D$
preserves the eigenspaces of $-D^2$. This is because $D$ commutes with
$-D^2$, and commuting operators preserve eigenspaces. Let $E_\lambda$ denote the
eigenspace of $D^2$ with eigenvalue $\lambda^2$. Again, since the right Clifford action
commutes with $D$, these eigenspaces are preserved by the right Clifford action,
giving them the structure of right $\Cliff_{n,0}(\R)$ modules. In addition, the
operator $F_\lambda = D / \lambda\vert_{E_\lambda}$ satisfies
$F_\lambda^2 = D^2 / \lambda^2 = -\id_{E_\lambda}$, and commutes with the right
Clifford action
\fi
%