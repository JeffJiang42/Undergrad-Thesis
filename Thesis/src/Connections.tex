...
The correspondence $\Omega^k_M(E) \leftrightarrow \Omega^{k,G}_P(V)$ gives
...
\end{defn}
%
We are especially interested in the restriction of the $\nabla_\Theta$ to
the subspace $\Omega_P^{k,G}(V) \cong \Omega_M^k(E)$, where $E$ denotes
the associated vector bundle $P \times_G V$. The first thing to check
is that given $\alpha \in \Omega_P^{k,G}(V)$, its image $\nabla_\Theta\alpha$ is 
an element of $\Omega_P^{k,G}(V)
%
