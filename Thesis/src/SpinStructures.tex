...
\end{exmp}
%
In fact, every sphere is a Spin manifold
%
\begin{thm}
For all $n \in \Z^{\geq 0}$, $S^n$ is a Spin manifold.
\end{thm}
%
\begin{proof}
...
orientation on $\R^{n+1}$. We use this to define a fiber bundle
$\pi : SO_{n+1} \to S^n$ with fiber $SO_n$. Given a matrix $A \in SO_{n+1}$, the
...
standard double cover. The composition $\rho \circ \pi : \Spin_{n+1} \to S^n$ is
a principal $\Spin_n$ bundle over $S^n$, where the fiber over $p \in S^n$ is the
subgroup of $\Spin_{n+1}$ isomorphic to $\Spin_n$ that covers the fiber of
$\pi\inv(p)$, giving the desired reduction of structure group to $\Spin_n$.
\end{proof}
%
In particular, the Spin structure over $S^2$ is the Hopf fibration $S^3 \to S^2$,
since $SO_3 \cong \RP^3$.\\
%
