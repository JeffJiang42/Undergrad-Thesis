...
function $f$, which is the representation of the section with respect to the
...
ginving us that the Dirac operator $D_1$ on sections of $P_1 \times_{\Spin_1} \C$
is just $i\frac{d}{d\theta}$. For the connected double cover, we identified
...
\end{align*}
%
In the local trivialization $\sigma(\theta) = e^{i\theta/2}$, the operator $D_2$
operates on $2\pi$-periodic functions, just like $D_1$, and is given by
$D_2 = \partial_\theta - \frac{1}{2}$. In particular, the first operator $D_1$ has
